                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        @foreach($predefinedOptions as $option)
                            <label class="flex items-center">
                                <input type="checkbox" name="selected_unit_options[]" value="{{ $option }}" {{ in_array($option, $selectedOptions) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
