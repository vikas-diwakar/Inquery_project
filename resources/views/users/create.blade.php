...
@section('title', 'Create User - PropDrip')
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-slate-200/80 overflow-hidden">
        ...
        <div class="bg-slate-900 p-6 sm:p-8 text-white flex items-center justify-between relative overflow-hidden">
            ...
            <div class="space-y-1 relative z-10">
                <h1 class="text-xl sm:text-2xl font-extrabold tracking-tight">Create New User</h1>
                <p class="text-xs sm:text-sm text-slate-300">Add a team member to your company workspace and configure permissions.</p>
            </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-1.5">
                    <label for="password" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider">Password <span class="text-rose-500">*</span></label>
                    <div class="relative rounded-xl shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-1.5">
                    <label for="password_confirmation" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider">Confirm Password <span class="text-rose-500">*</span></label>
                    <div class="relative rounded-xl shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Projects Checkboxes Field (Only shown for non-admin roles) -->
                        ...
                        @forelse($projects as $project)
                            <label for="project_{{ $project->id }}" class="flex items-center p-3 rounded-xl bg-white border border-slate-200/80 hover:border-indigo-300 transition-all cursor-pointer space-x-3">
                                <input type="checkbox" name="project_ids[]" value="{{ $project->id }}" id="project_{{ $project->id }}" 
                                    {{ in_array($project->id, old('project_ids', session('selected_project_id') ? [session('selected_project_id')] : [])) ? 'checked' : '' }} 
